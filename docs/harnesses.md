...
| Dual-storage contract suite | Vitest + sql.js (real SQLite in Node) | 🔜 Planned (Phase B) | Web (`localStorage`) vs native (SQLite) repo parity + DB drift vs types |
...
## Phase B — Dual-storage contract suite (planned)
...
- **DB drift test**: after running migrations on sql.js, `PRAGMA table_info` must match
  `src/database/types.ts` and the web storage fields; seed row counts must match
  `src/database/seedData.ts`. This catches a column added to a migration that types or web
  storage forgot.

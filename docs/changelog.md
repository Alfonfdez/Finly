...
[2026-09-04] Refactor | Style-guide token pass (Option C, Feature 2): theme-driven ErrorBoundary + border-radius token consolidation
- Fixed the one real theme-drift bug: `ErrorBoundary.tsx` was a class component that hardcoded `darkColors.*` for its fallback UI, so it stayed dark even under the light theme. Rewritten as a functional wrapper (`useConfig().activeColors`) around an inner `ErrorBoundaryBase` class that keeps the lifecycle hooks (`getDerivedStateFromError`/`componentDidCatch`); error UI now follows the active theme (`background`/`text`/`textSecondary`/`primary`) via inline style arrays. Used in `App.tsx` inside `ConfigProvider`, so the hook is valid.
- Confirmed zero frozen palette hex colors in any StyleSheet: all raw hexes are legit (DB color defaults `#22D3EE`/`#A78BFA`, national flag colors in RegionalScreen, rgba overlays/shadows); `WHITE` is used only as a foreground and `BLACK` only as a shadow. No colors were changed.
- Added two shared radius tokens to `src/components/componentStyles.ts`: `CONTROL_BORDER_RADIUS = 8` and `CARD_BORDER_RADIUS = 12` (joining existing `MODAL_BORDER_RADIUS = 16`, `BUTTON_BORDER_RADIUS = 10`, `PILL_RADIUS = 999`, `OVERLAY_BG`).
- Consolidated repeated `borderRadius` literals into the tokens across components, shared style files, screens and the navigator drawer (value-preserving: `8`->CONTROL, `10`->BUTTON, `12`->CARD). Files touched: `formStyles.ts`, `calendarStyles.ts`, `settingsStyles.ts`, `SearchBar`, `TabBar`, `AmountInput`, `DaySelector`, `BarChart`, `CommentInput`, `PeriodTabs`, `SelectionActionBar`, `PhotoSection`, `TagSection` (3x), `TransactionForm`, `CategoryFilterModal`, `ColorPickerModal`, `CategoryTransferModal` (2x), `CalendarPicker`, `CalendarModal` (2x), `CalculatorModal` (2x), `AccountForm`, `DeleteButton`, `SettingsPickerRow`, `WeekPicker`, `PeriodPicker`, `DataScreen`, `CreateTagScreen`, `ModifyCategoryScreen`, `ModifyCommentScreen`, `ModifyTagScreen`, `TransactionDetailsScreen` (3x), `AccountsScreen`, `AppNavigator`.
- One intentional visual change (declared): `TagChip` chip radius `8` -> `MODAL_BORDER_RADIUS` (16), so chips are rounder and consistent with the other chip-family radius. Left as-is by design: circle/avatar radius derived from half of a size (e.g. `removeButton` 22/2 = 11, `legendColor` 10/2, `slider` 15), and chip-family `16` literals.
- Tests: new `tests/component/ErrorBoundary.test.tsx` (5 tests) covering renders children when nothing throws, shows the fallback when a child throws, drives the fallback background from the dark theme, updates the background when the theme switches to light via `setColors`, and renders the retry button with the primary color and fires `onRetry`.
- Verification: `test:all` green - typecheck + lint + 73 files / 457 tests (was 72 files / 452 tests).

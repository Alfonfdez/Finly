...
[2026-08-30] Refactor | Search/select dedup (shared ScreenSearchBar, useSelectableScreen header re-emission, empty-state helper)
- All 6 list screens (Tags, Comments, Categories, Accounts, AllTransactions, AddCategory) now use `ScreenSearchBar` (`visible`/`autoFocus`/`onClose`) instead of hand-rolled `{searchActive && <SearchBar autoFocus/>}` blocks. `ScreenSearchBar` gained an optional `style` so Categories/AddCategory (which render inside a 16px-padded content) keep their exact current padding without doubling the horizontal inset.
- `AddCategoryScreen` dropped its local search state + `headerRightRef`/`useLayoutEffect` one-shot in favor of `useSelectableScreen` (search-only header). The raw `SearchBar` import is gone from all screens.
- `useSelectableScreen` now re-emits `navigation.setOptions` when `selectMode` changes (`select.selectMode` added to the effect deps): native-stack headers only re-render on a `setOptions` emit, so toggling select mode previously left the header button stale (checkbox "Enter select mode" icon instead of the close "Exit select mode" icon) on every screen using the hook. Browser-verified in Accounts/Categories/AllTransactions: the header flips between the two states.
- `AllTransactionsScreen` keeps its conditional header (buttons only when the filtered list has sections) with its own gated `useLayoutEffect`, because the gate value (`hasSections`) is derived after the hook call and can't be expressed through `useSelectableScreen`. This also restored the buttons after data load: the initial hook adoption had broken them (the one-shot `setOptions` registered a header that read `hasSections=false` from the first render and never re-emitted).
- Added `emptyStateProps(searchActive, idleIcon, idleMessage)` to `EmptyState.tsx`, used in Tags, Comments, Categories and AllTransactions: a failed search now shows `filter_no_results` ("No results found") instead of the screen's idle message. Fixes a real bug in `CommentsScreen`, which always showed "No comments yet" even after a failed search; CategoriesScreen now also flips correctly during search.
- Verified in the browser at 375px (viewport): Categories/AllTransactions/AddCategory search (open, filter, no-results state, close restores list); AllTransactions header buttons after load + select-mode toggle; Accounts select-mode header toggle; Comments failed search now shows "No results found". test:all green (typecheck + lint + 283 tests, 34 files).

[2026-08-30] Fix | utils/search.ts matchesAllTerms case-insensitive haystacks
- User report: searching a list by prefix showed nothing on the Comments, Tags, Categories (and AddCategory) screens even though matching items existed (e.g. comment "Baena", typing "B" or "Ba"); All Transactions search worked fine.
- Root cause: `matchesAllTerms(query, ...haystacks)` lowercased the query via `searchTerms` but compared each term with `haystack.includes(term)` against the RAW haystack, so "Baena".includes("b") was false and every capitalized row was filtered out. All Transactions and Accounts worked only because their helpers (`matchesTransactionSearch`, `matchesAccountSearch`) pre-lowercase the haystacks before delegating; Tags/Comments/Categories/AddCategory pass raw strings through `useSearchFilter`, which surfaced the bug.
- Fix: `matchesAllTerms` now lowercases each haystack at compare time (`haystack.toLowerCase().includes(term)`), so every caller — account/transaction helpers and the four `useSearchFilter` screens — is case-insensitive regardless of haystack case. Pre-lowering at the existing call sites is now redundant but harmless (left untouched, zero churn).
- Tests: new `tests/utils/search.test.ts` (10 tests) covers `searchTerms` tokenization and `matchesAllTerms` (empty query, case-insensitive match, substring at any position, multi-word AND, multi-haystack OR, no match).
- Verified in the browser at 375px (fresh IndexedDB seed): seeded comment "Baena" by adding a 5 EUR Groceries expense, then Comments search "B" and "Ba" both keep the Baena row and "zzz" shows "No results found"; created tag "Work", Tags search "W" keeps it; Categories search "gro" keeps only Groceries; All Transactions search "gro" still keeps the Baena/Groceries transaction (no regression). test:all green (typecheck + lint + 293 tests, 35 files).

[2026-08-31] Refactor | dead-code cleanup (unused exports and dead language/platform guards)
- Trimmed exports that were never imported anywhere in the codebase (src/ + tests/): dropped the `export` keyword on internal-only symbols used solely within their own file — `isValidHexColor` (utils/color.ts, only used by `withAlpha`), `IconColorHintKeys` (utils/formHints.ts, only used by `getIconColorHintText`), `FitFontSizeOptions` and `AmountSign` (utils/formatters.ts; `AmountSign` was fully dead — removed entirely, no internal use), `PICKING_PHASE` + `PickingPhase` (components/calendars/PeriodPicker.tsx, used ~9 times internally only), `CurrencyLabelKey` (constants/currencies.ts, only used by `CurrencyOption`). Removed the dead `export type { TransactionDraft }` re-export in `components/TransactionForm.tsx` (the type stays imported and used internally by its Props interface).
- Deleted read-only convenience predicates that no module imports: the six language guards that aren't in the app's 3-language set (`isEnglish`, `isSpanish`, `isFrench`, `isGerman`, `isPortuguese`, `isItalian`) in `utils/language.ts` (`isCatalan` is live in RegionalScreen and was kept, as were the heavily-used `LANGUAGES`/`Language`), plus `isAndroid`/`isIOS` in `utils/platform.ts` (only `isWeb`/`isNative` are used; the `isAndroid` string in a test is an unrelated mock property name).
- No behavior change. Verified in the browser at 375px: Tags (Work row + "1 of 50 tags" counter), Comments (Baena row), Settings > Regional (language/currency/separator/calendar options render — confirms `isCatalan` import intact), and Home > Add expense or income (TransactionForm renders). test:all green (typecheck + lint + 293 tests, 35 files).

[2026-08-31] Fix | DataScreen.tsx - remove web default focus outline on modal text inputs
- User report: on web, the confirm "Type DELETE" text box in Settings > Data (delete-all-data and factory-reset modals) showed a light ring/area around it when focused that was cropped on the left and right sides; it worked fine on mobile.
- Root cause: these are the only `TextInput`s in the app rendered inside a modal (`ConfirmationModal`). React Native Web renders them as `<input>`, which the browser paints a default focus outline on (`outline: ... auto 1px`). The input spans the modal content width exactly, flush against `ConfirmationModal`'s `ScrollView` (`overflowX: hidden`), so the outline's left/right 1px edges are clipped by that overflow — the broken ring the user saw. Native has no focus ring, and form inputs (e.g. Add Transaction) are inset by the 16px scroll-content padding, so their outline renders without clipping — which is why only modal inputs showed the issue.
- Fix: a web-only `outlineStyle: 'none'` on the two confirm `TextInput`s (appended via a local `webInputFocusReset` style, `isWeb`-gated, `null` on native so it's a no-op there). `outlineStyle` is supported by react-native-web at runtime but not in RN's TS `TextStyle` type, so it's applied with the codebase's existing `as unknown as` cast pattern. No changes to `ConfirmationModal`/`ModalShell` or form inputs.
- Verified in the browser at 375px: focused "Type DELETE here" input now computes `outline-style: none` (was `auto`), with its own `borderColor` intact; Add Transaction Comment input still computes `outline-style: auto` (unaffected, confirms the fix is scoped to modal inputs as reported). test:all green (typecheck + lint + 293 tests, 35 files).

[2026-08-31] Refactor | utils/errors.ts runWithErrorAlert - dedupe repetitive try/catch error handling
- Added `runWithErrorAlert(fn, prefix, labels?)` to `src/utils/errors.ts`: wraps an async operation in try/catch that, on failure, logs `console.error('<prefix>:', err)` and calls the existing `showErrorAlert(labels)`, returning `undefined` on error (mirrors what each call site already did manually).
- Migrated the 13 uniform `catch (err) { console.error('Failed to ...', err); showErrorAlert(labels); }` blocks across 10 screens to the helper: CommentsScreen (delete comments), TagsScreen (delete tags), CategoriesScreen (permanent delete + confirm resolution), AccountsScreen (bulk delete), CreateAccountScreen (create), CreateTagScreen (create), CreateCategoryScreen (create), ModifyAccountScreen (update + delete), ModifyCommentScreen (update + delete), ModifyCategoryScreen (update + permanent delete + reassign-and-delete).
- Left intentionally untouched (non-uniform: extra catch logic or no console.error, so not a drop-in for the helper): ModifyTagScreen (catch calls showErrorAlert without console.error), TransactionDetailsScreen (catch has extra setDeleting/setDeleteModalVisible cleanup), AllTransactionsScreen (catch omits console.error), and the context-layer handlers in AppContext/ConfigContext.
- No behavior change. Removed the now-stale `labels` dep from AccountsScreen's handleBulkDelete useCallback. Verified in the browser at 375px (fresh IndexedDB seed): created tag "SmokeTest" via Create Tag (CreateTagScreen) -> row appeared (2 of 50 tags); bulk-selected and deleted it via the delete-confirm modal (TagsScreen) -> back to 1 of 50 tags, select mode exited, no error alert, 0 console errors. test:all green (typecheck + lint + 293 tests, 35 files).

[2026-08-31] Refactor | hooks/useBulkDelete - extract shared bulk-delete orchestration
- Added `useBulkDelete({ selectedIds, exitSelectMode, deleteFn, afterDelete?, errorPrefix, labels? })` to `src/hooks/useBulkDelete.ts`: owns the delete-confirm modal state (`deleteModalVisible`) plus `openDeleteModal`/`closeDeleteModal`, and a single `confirmBulkDelete` that closes the modal, runs `runWithErrorAlert(async () => { await deleteFn([...selectedIds]); exitSelectMode(); await afterDelete?.(); }, errorPrefix)` (reusing the error helper added in the prior step).
- Migrated the near-identical `handleBulkDelete` + confirm-modal state from three screens to the hook (the JSX `SelectionActionBar`/`ConfirmationModal`/`Fab` wiring stays in each screen, since the non-select-mode fork differs per screen): TagsScreen (deleteMany + refreshTags), CommentsScreen (deleteComments + loadComments), AllTransactionsScreen (deleteMany + its custom reload: `list({})` -> `setAllTransactions` -> `refresh()` passed as `afterDelete`).
- Behavior notes: AllTransactions' previous catch body only called `showErrorAlert` (no console.error); routing it through `useBulkDelete`/`runWithErrorAlert` now also logs `console.error('Failed to delete transactions:', err)` on the failure path, matching every other screen and purely additive (invisible to users). Its success-path post-delete reload is preserved verbatim via `afterDelete`.
- Left intentionally out of scope (different orchestration, per the S1b plan): CategoriesScreen (transfer-vs-delete resolution + `bulkDeleteWithTargets`), AccountsScreen (`sanitizeDefaultAccounts` config update + reload + refresh inside the callback), and the single-entity Modify* screens.
- No behavior change on any migrated success path. Verified in the browser at 375px (fresh IndexedDB seed) for all three migrated flows: Tags (created "SmokeS1b" -> select mode -> Delete (1) -> confirm -> back to 1 of 50, select mode exited); Comments (selected seeded "Baena" -> Delete (1) -> confirm -> empty state "No comments yet", select mode exited); AllTransactions (selected seeded Groceries -5,00 e -> Delete (1) -> confirm -> Total recomputed to +0,00 e, empty state "No transactions", select mode exited). 0 console errors throughout. test:all green (typecheck + lint + 293 tests, 35 files).

[2026-09-01] Refactor | components/settings/SettingsSection - shared settings section wrapper
- Added `SettingsSection` (`src/components/settings/SettingsSection.tsx`): moves a settings section's header (`settingsStyles.section`, `textSecondary` title at `fs(12)`) and optional `settingsStyles.card` wrapper (`card` prop, default `true`) out of each screen into one shared component; internalizes the `useConfig`/`useFontSize` reads needed for those styles.
- Migrated all 10 section-header sites across the three settings screens to `<SettingsSection>`: AppearanceScreen (4 sites, default `card={true}`; dropped now-unused `View`, `useFontSize`/`fs` imports), RegionalScreen (3 sites, `card={false}`; `View`/`Text`/`useFontSize` kept for the option icons), PersonalizationScreen (3 sites, `card={false}`; the inline "Optional fields" and "Hide account balances" card children stay as explicit `<View>` inside their section).
- No behavior or style change. Verified in the browser at 375px for all three screens (Section: headers + content render correctly, card-wrapped vs card-less sections as before, inline cards intact). 0 console errors (remaining `pointerEvents` deprecation warning is pre-existing and unrelated). test:all green (typecheck + lint + 293 tests, 35 files).

[2026-09-01] Fix | components/CategoryGrid - center grid rows
- Added `justifyContent: 'center'` to `CategoryGrid`'s `styles.grid` (flex-direction row + flex-wrap). Previously wrapped lines defaulted to `flex-start`, so a partial final row (e.g. the Categories screen's last row with 2 tiles: "Others" + "Create") was pinned to the left, leaving empty space on the right.
- With flex-wrap + `justifyContent: 'center'` each wrapped row line is centered independently: full 4-tile rows keep their layout (shift ~1px from left-aligned), and partial last rows become centered. Layout-only change; no colors/tokens affected.
- Applies uniformly to the three shared `CategoryGrid` usages: CategoriesScreen, AddCategoryScreen, TransactionForm (transaction form category picker). Verified in the browser at 375px: Categories screen last row ("Others"/"Create") now centered symmetrically (full rows start at x=25, last row at x=108 within a 343px grid), and the transaction-form grid (8 tiles, 2 full rows) also centers. 0 console errors. test:all green (typecheck + lint + 293 tests, 35 files).

[2026-09-01] Refactor | Tags/Comments - consolidate rows onto ListItemRow
- Replaced the hand-rolled selectable row (TouchableOpacity + checkbox + text + chevron) in TagsScreen and CommentsScreen with the shared `ListItemRow` component, matching the pattern AccountsScreen already uses.
- TagsScreen: `title={item.name}` (`titleSize={15}`), select-checkbox passed via `leading`, chevron via `right` (both conditionally rendered on `selectMode`), `divider` for the bottom border, `style={{ backgroundColor: c.surface, padding: 14 }}`. Removed the `row` (layout props now built-in), `tagName` and `checkbox` style blocks, dropped the `TouchableOpacity` import, and removed the now-unused `fs` from the renderItem useCallback deps (title size is static).
- CommentsScreen: same migration, with the two-line middle (comment + `labels.comments_used_in(count)`) replaced by `title` + `subtitle` (`ListItemRow` already renders subtitle at `fs(12)`). Removed the `row` layout props, `rowText`, `commentText`, `commentCount` and `checkbox` style blocks, and dropped the `TouchableOpacity` import.
- Layout notes: row vertical padding now `14` via the `style` prop; the checkbox gap is `12` (`ListItemRow` default `badgeGap`) versus the previous 10 — an imperceptible 2px change, consistent with other ListItemRow usages. `accessibilityRole="button"` dropped (TouchableOpacity is natively interactive). No behavior change.
- Verified in the browser at 375px (fresh IndexedDB seed): Tags list view renders the row with chevron; entering select mode swaps the chevron for a left checkbox and shows the SelectionActionBar (Delete (0)). Comments screen renders (empty state here; uses the identical shared ListItemRow as Tags/Accounts). 0 console errors (pre-existing `pointerEvents` deprecation warning only). test:all green (typecheck + lint + 293 tests, 35 files).

[2026-09-01] Refactor | ModifyTag - consolidate save/delete onto runWithErrorAlert
- Replaced the two remaining direct try/catch + showErrorAlert blocks in ModifyTagScreen (handleSave, handleDelete) with the shared runWithErrorAlert helper, completing the N1 error-handling consolidation story for entity save/delete screens (now matches CreateTag, CreateCategory, CreateAccount, ModifyAccount, ModifyCategory, ModifyComment).
- Prefixes follow the existing convention: 'Failed to update tag' / 'Failed to delete tag' (mirrors 'Failed to create tag' in CreateTagScreen and the ModifyCategory update/delete prefixes).
- The helper adds a console.error with the prefix on failure (purely additive, invisible to users); showErrorAlert import fully removed from the file. No behavior change on the success path.
- Explicitly out of scope (documented rationale): ModifyCommentScreen load catch, TransactionDetailsScreen delete catch (has extra state resets), and the filter onError one-liners in TransactionsScreen/AllTransactionsScreen/TransactionForm (not try/catch blocks).
- Verified in the browser at 375px (fresh IndexedDB seed): renamed the seeded 'Work' tag to 'Work2' via Save (list updated, navigated back), then reopened and deleted it via the ConfirmationModal (empty state '0 of 50 tags' shown). 0 console errors. test:all green (typecheck + lint + 293 tests, 35 files).

[2026-09-01] Refactor | Error prefixes - centralize runWithErrorAlert/useBulkDelete log prefixes
- Introduced an exported typed `ERROR_PREFIXES` const in src/utils/errors.ts (the same module that defines runWithErrorAlert), with all 16 developer-only console.error prefixes: account/category/tag/comment create-update-delete (singular) plus accounts/categories/tags/comments/transactions bulk-delete (plural).
- Migrated every call site from inline string literals to the const: CreateAccountScreen, ModifyAccountScreen, AccountsScreen, CreateCategoryScreen, ModifyCategoryScreen (x3), CategoriesScreen (x2), CreateTagScreen, ModifyTagScreen (x2), TagsScreen, CommentsScreen, AllTransactionsScreen, ModifyCommentScreen (x2). CommentsScreen and TagsScreen gained a new utils/errors import; AllTransactionsScreen/AccountsScreen/ModifyCommentScreen extended their existing named imports.
...
[2026-09-02] Feature | Transactions screen - search and multi-select bulk delete
- TransactionsScreen (014) now matches the AllTransactions implementation: top-right `SelectSearchHeader` (selection toggle + search icon) registered via `navigation.setOptions` in a `useLayoutEffect`, both hidden when the screen has 0 transactions via a `headerRightRef`.
- Search: `ScreenSearchBar` toggling below the category section; client-side case-insensitive multi-term (AND) over comment/description, category display name, tag names and account name via `useTransactionFilters({ searchTerm: searchText, categoriesById })`; composes with the account selector and tag filter; "No results found" empty state via `emptyStateProps`.
- Select: `useSelectAndSearch` + `useBulkDelete`; transaction rows toggle selection instead of navigating (`selectMode`/`selected` on `TransactionRow`); `SelectionActionBar` (Cancel / `Delete (N)`) swaps the FAB; single `ConfirmationModal`; `transactionRepository.deleteMany` cleans photos + junction rows, then reloads via `setData` + AppContext `refresh`.
- FAB restored on this screen (spec §5) and navigates to AddTransaction with the route `type`.
- Updated spec/features/014-transactions-screen-from-home/1-spec.md (objective, two new sections "Transaction search" + "Multi-select bulk delete", NFR layout string, 12 new acceptance criteria flipped [x]) and spec/constitution/3-roadmap.md (new entry `## 2.1 transactions screen search and select`, Status: completed.).
- Verified in the browser at 375px: created Groceries "coffee" 10,00 and "bread" 5,00 plus Transport "bus" 3,00; drilled Home -> Groceries -> Transactions; search "coffee" narrowed to 1 row (total updated to -10,00), search "coffee xyz" showed "No results found" (total +0,00), close search restored both rows; Select mode -> checkboxes on both -> Delete (2) -> "Delete 2 transactions?" confirm -> both removed, total +0,00, empty state "No transactions", FAB restored, header Select/Search hidden at 0 rows. 0 console errors. test:all green (typecheck + lint + 334 tests, 40 files).

[2026-09-02] Feature | Transactions screen - show active period in header
- The Transactions screen (014) filters data by category AND period (day/week/month/year/custom) but only surfaced the category; added a Row 3 period indicator sub-line in the category header so the user always sees the active period and understands why an out-of-period transaction doesn't appear.
- New `periodLabel` useMemo in TransactionsScreen builds `<period label> · <date/range>` from the route params (`period`, `startDate`, `endDate`): Day -> `formatDateLong` (e.g. "Day · September 2, 2026"); Week -> start–end range via `formatDateLong`; Month -> localized month + year via `getMonthName` (e.g. "Month · September 2026"); Year -> the year; Custom -> start–end range via `formatDateLong`. Period labels reuse existing `period_*` i18n keys; dates are localized per active language (`config.language`). Parses route dates with `parseDbDate`.
- Added the `categoryPeriod` style (secondary color, fs(13), centered under the total). No new i18n keys, no icons, no period-changing interaction (purely informational).
- Updated spec/features/014-transactions-screen-from-home/1-spec.md (category section Row 3 with per-period formats, new acceptance criterion) and spec/constitution/3-roadmap.md (2.1 entry extended with the period indicator note).
- Browser-verified (Playwright, 375px): Transactions header shows the period line correctly for Day ("Day · September 2, 2026"), Month ("Month · September 2026") and Custom range ("Period · January 1, 2026 – September 2, 2026"), localized; 0 console errors. test:all green (typecheck + lint + 334 tests, 40 files). 014 criteria flipped [x].

[2026-09-02] Feature | Home period picker - explain the "All" range
- The custom period picker (PeriodPicker, used by Home's "Period" tab) had an "All" checkbox that means "the whole current year so far" (Jan 1 of the current year `MIN_DATE` to today), but the range text above the picker only showed the literal "All" label with no dates, leaving the user unsure what period that covers.
- Changed PeriodPicker's `rangeText` so that when `allTime` (All) is checked it shows the resolved range explicitly: `All (from 1 Jan to 2 Sep 2026)`, e.g. `"${labels.cal_all} (${labels.cal_from} ${MIN_DATE.getDate()} ${shortMonths[MIN_DATE.getMonth()]} ${labels.cal_to} ${today.getDate()} ${shortMonths[today.getMonth()]} ${today.getFullYear()})"`. Reuses the existing localized `cal_all`/`cal_from`/`cal_to`/`months_short` keys; no new i18n keys, no new computation (MIN_DATE + today already in scope).
- The non-All custom-range text is unchanged and still shows `from 1 Jan to 2 Sep 2026`.
- Updated spec/features/001-home-screen/1-spec.md (period picker bullet + new acceptance criterion) and spec/constitution/3-roadmap.md (001 entry note).
- Browser-verified (Playwright, 375px): checking "All" shows `All (from 1 Jan to 2 Sep 2026)` and hides the day grid; unchecking returns to `from 1 Jan to 2 Sep 2026` with the "Select start date" grid. 0 console errors. test:all green (typecheck + lint + 334 tests, 40 files). 001 criteria flipped [x].

[2026-09-02] Refactor | Unify the two transaction list screens
- `AllTransactionsScreen.tsx` and `TransactionsScreen.tsx` shared ~60% duplicated logic: the `useSelectAndSearch` + `useLayoutEffect` header wiring, the `useBulkDelete` wiring with the identical afterDelete reload body, the `useTransactionFilters` invocation, and the identical `handleTransactionPress`/`keyExtractor` callbacks.
- New shared hook `src/hooks/useTransactionListScreen.tsx` now owns the common plumbing: `useTransactionFilters`, `useBulkDelete` (with the shared afterDelete that reloads via `loadTransactions` → `setTransactions` → `onAfterDelete`), plus `handleTransactionPress` and `keyExtractor`. Both screens now pass their screen-specific pieces (load function, filter options, delete fn, refresh callback) into the hook.
- Both screens moved off the raw `useSelectAndSearch` + `useLayoutEffect` header wiring onto the shared `useSelectableScreen` hook, making them consistent with `CategoriesScreen`/`AccountsScreen`/`TagsScreen`/`CommentsScreen`. Header visibility (`showHeader`) is now driven by the loaded transaction count (`allTransactions.length > 0`), matching the pre-existing "hide Select/Search when there are no transactions" criterion.
- Screen-specific parts stay in the screens: category banner + route-filtered loading + `TabBar`/category-filter/period tabs + global `selectAccount` side-effect in the account modal. Component-dependent render helpers (`renderItem`/`renderSectionHeader`) also stay in the screens — the project's lint `boundaries/dependencies` rule forbids hooks from importing components, so these live with their component imports.
...
[2026-09-03] Tests | Tier-3 completion: useTransactionForm hook suite
- New `tests/hooks/useTransactionForm.test.tsx` (14 tests): closes the last in-scope Tier-3 coverage gap. Mocks `../../src/database` (`getTagsByTransactionId`, `getCategoryUsageCounts`, `tagRepository.create`), `@react-navigation/native` (async factory with `React.useEffect` for `useFocusEffect`), appStub, `expo-image-picker` and `expo-file-system` (the latter two required because the hook pulls in `usePhotos`). Builds a typed `UseTransactionFormProps` locally (the hook does not export its props type).
- Covers: canSubmit gating (amount + account), initial amount parse, type/account/date/category setters, tag toggle, `handleCreateTag` success/duplicate/error paths (with refreshTags), `handleSubmit` draft shape + goBack + refresh, guarded submit on invalid state, tag preload via `transactionId`, and category-usage ordering of `visibleCategories`.
- Verification: `test:all` green - typecheck + lint + 53 files / 403 tests (was 52 / 389). No source code or spec criteria changed (tests only). Tier-3 coverage pass 1 is now complete.
[2026-09-03] ~ | FinlyApp/src/i18n/de.ts
- Tier-4 item 9: reformatted `de.ts` to match the section-comment style of `en.ts`/`fr.ts`/`it.ts`/`pt.ts` by inserting the same 33 `// Section` header comments (Generic, Settings, Tabs, Category filter, Periods, Calendar modal, Calendar picker display, Day/month abbreviations, Home, Account modal, Add/Modify/Create category, Categories, Accounts, Transactions, Navigation, Day letters, Currencies, Languages, Accessibility, Calculator, Transaction details, Modify transaction, Tags, Comments, Home tag filter, Color grid, Accessibility labels). No translation values changed.

[2026-09-03] ~ | FinlyApp/src/screens/settings/DataScreen.tsx + FinlyApp/tests/screens/DataScreen.test.tsx
- Tier-4 item 10: extracted a local `ConfirmWithTextModal` helper in `DataScreen.tsx` that wraps `ConfirmationModal` with the shared "type DELETE to confirm" `TextInput` + `confirmDisabled` gating, replacing the two near-identical `deleteAllModal2` / `factoryResetModal2` blocks (~30 duplicated JSX lines). Removed the now-unused `canDeleteAll`/`canFactoryReset` locals and parent `useFontSize`.
- Added `tests/screens/DataScreen.test.tsx` smoke test (3 tests): renders the five data actions, confirms "delete all transactions" (calls `deleteAllTransactions` + `resetAll`), and verifies the "delete all data" two-step flow requires typing DELETE before the confirm is enabled (uses `userEvent.press` + awaited `fireEvent.changeText`, since RNTL 14 returns promises). Mocks `../../src/database`, `../../src/database/database`, `../../utils/backupIO`, appStub and `expo-file-system`.
- Verification: `test:all` green - typecheck + lint + 54 files / 406 tests (was 53 / 403).

[2026-09-03] Refactor | Tier-2 repo parity: category/tag getById + drop redundant junction delete
- Added `getById(id)` to `categoryRepo` and `tagRepo` (mirroring `accountRepo` and `transactionRepo`), each returning the parsed entity or `null` via `parseRowOrNull`. `ContractCategoryRepo` / `ContractTagRepo` in `tests/database/contractTypes.ts` now declare `getById` for interface parity, so the contract suite runs it against the native sqlite backend.
- Added contract coverage: `tests/database/contractSuite.ts` now verifies `category.getById` (read-back + null case) and `tag.getById` (read-back + null case).
- `transactionRepo.deleteAllTransactions` no longer explicitly deletes `transaction_tags` rows - the junction relies on `REFERENCES transactions(id) ON DELETE CASCADE` (migration `001_initial`, with `PRAGMA foreign_keys = ON`), matching the already-normalized `delete`/`deleteMany` paths. The existing contract test "deleteAllTransactions clears transactions and links but keeps accounts and tags" still passes, confirming the cascade clears the links.
- AccountsScreen/CategoriesScreen `useBulkDelete` unification intentionally dropped: both screens have richer delete flows (accounts: guard + config sanitization; categories: in-use check + transfer resolution) that `useBulkDelete`'s simple `deleteFn` contract cannot express, so keeping their explicit state is the correct design.
- No spec criteria changed (refactor only). test:all green - typecheck + lint + 54 files / 408 tests (was 54 / 406).

[2026-09-04] Tests | Tier-3 screen smoke tests (Option B, Phase 1): lists, home, details and settings
- Added 9 screen smoke suites in `tests/screens/`: `TagsScreen`, `CommentsScreen`, `AccountsScreen`, `TransactionDetailsScreen`, `HomeScreen` (lists/home/details), and `SettingsScreen`, `AppearanceScreen`, `PersonalizationScreen`, `RegionalScreen` (settings). Each is a render/basic-state smoke plus one core interaction that is testable without the navigator header:
  - Tags: row `navigate('ModifyTag', { tagId })`; Comments: row `navigate('ModifyComment', { comment })`; Accounts: row `navigate('ModifyAccount', { accountId })`.
  - TransactionDetails: delete flow through the `ConfirmationModal` (async `userEvent.press` + `findByRole('button', { name: 'Yes' })`, asserts `transactionRepository.delete(id)` + `navigation.goBack`); mocks `expo-file-system` for `photoUtils`, and `react-native-svg` (inline mock) is not needed here.
  - Home: fab opens `AddTransaction`; mocks `transactionRepository.breakdownByCategoriesAndTags` and `react-native-svg` (DonutChart uses the default `Svg` export, so the inline mock returns both `default` and named `Svg`/`G`/`Circle`/`Path` stubs, kept fully inside the `vi.mock` factory to avoid the hoisted-factory TDZ).
  - Settings: rows render + open `SettingsAppearance`; Appearance selects 'Light' -> `updateConfig({ theme: 'light' })` via `SelectorInline`; Personalization toggles 'Hide account balances' -> `updateConfig({ hideBalances: true })` (ToggleRow); Regional is render-only (language/currency/format options; its pickers open modals, so left as a smoke render; labels like 'LANGUAGE' and 'Default account' appear twice - section header + row label - so those asserts use `getAllByText`).
- Select-mode / bulk-delete flows are deliberately not exercised through the header button: the select-mode entry lives in the navigator `headerRight` (set via `useSelectableScreen` -> `navigation.setOptions`), which the screen-only `@react-navigation/native` mock does not render (the existing `TransactionsScreen`/`AllTransactionsScreen` suites also render-only). Bulk-delete logic itself remains covered by the `useBulkDelete`/`useSelectAndSearch` hook suites.
- Fixed a latent hang in the shared `tests/component/helpers/appStub.ts`: `buildAppMock()` returned a fresh `activeCategories: []` on every call, so any screen with an effect depending on `activeCategories` (HomeScreen's tag-breakdown loader) re-ran the effect forever -> infinite render loop that hung the whole vitest run. `activeCategories`, `activeTagIds`, `filteredTransactions` and `tagsByTransaction` are now stable module-level empty references, preserving identity across renders.
- Verification: `test:all` green - typecheck + lint + 63 files / 430 tests (was 54 / 408). Phase 2 (account/category/tag + transaction create/modify forms, needs `reanimated-color-picker`, `expo-image-picker`/`expo-file-system` mocks) still pending by plan.

[2026-09-04] Tests | Tier-3 screen smoke tests (Option B, Phase 2): account/category/tag + transaction create/modify forms
- Added 8 screen smoke suites in `tests/screens/`: `CreateTagScreen`, `ModifyTagScreen`, `CreateAccountScreen`, `ModifyAccountScreen`, `CreateCategoryScreen`, `ModifyCategoryScreen`, `AddTransactionScreen`, `ModifyTransactionScreen`. Each is a render/basic-state smoke plus one core interaction, mirroring Phase 1 patterns (nav mock with mutable `routeParams` for `useRoute`, `vi.mock('../../src/database')`, appStub + configStub, `useFocusLoad` via `React.useEffect`).
- Debounced-duplicate-check handling: `useNameDuplicateCheck` keeps `checkingName` true until the 300 ms debounce fires and `existsByName` resolves, so Create/Save stay disabled in between. Tests use `vi.useFakeTimers()` + `await fireEvent.changeText` (the async handler must be awaited so the state update flushes) + `await act(() => vi.advanceTimersByTimeAsync(300))` to resolve the check, then assert the button becomes enabled before pressing.
  - CreateTag: type name -> debounce resolves -> press Create -> `tagRepository.create({ user_id: USER_ID, name })` + `goBack`; also verifies Create is disabled before any name and right after typing (checking).
  - ModifyTag: preloads the tag name into the field (`getByDisplayValue`); core interaction is the delete flow via `ConfirmationModal` (async `userEvent.press` + `findByRole('button', { name: 'Delete' })` -> `tagRepository.delete(id)` + `goBack`).
- Color forms use a shared `reanimated-color-picker` mock (redirect-imports: `ColorPicker` renders its `children`, `Panel1`/`HueSlider`/`OpacitySlider`/`Preview` render null), fully inline in the `vi.mock` factory since `AccountForm`/`IconColorSection`/`ColorPickerModal` pull the lib in.
  - CreateAccount / CreateCategory: core interaction is typing a name -> the icon+color hint appears ('Select an icon and a color', `create_account_error_icon_color` / `create_cat_hint_icon_color`) and Create/Add stays disabled until an icon AND color are chosen (no icon/color grid driving needed).
  - ModifyAccount: preloads via `useFocusLoad(accountRepository.getById)` + `useEffect` (name/icon/color/initial balance); mocks `sanitizeDefaultAccounts` (delete path); core interaction is edit name -> debounce -> Save -> `accountRepository.update(id, { name, ... })` + `goBack`. `is_total: 0` keeps the name editable and Save enabled.
  - ModifyCategory: category resolved from appStub `categories`; mocks `categoryRepository.existsByName/update` and `transactionRepository.list`; core interaction is edit name -> debounce -> Save -> `categoryRepository.update(id, { name, icon, color })` + `goBack`. Delete/transfer-modal modal flows are left out of scope (guard logic), matching the plan.
- Transaction forms reuse the `useTransactionForm.test.tsx` mock set (`expo-image-picker`, `expo-file-system` with a `File` class + `mockFsFile`, `transactionRepository.getTagsByTransactionId/getCategoryUsageCounts/searchComments/createWithTags|updateWithTags`, `tagRepository.create`) plus a stubbed `requestAnimationFrame` and appStub accounts/categories/categoriesById/tags. `CategoryTile`/`IconBadge` need no svg mock (Ionicons, already aliased).
  - AddTransaction: renders the full TransactionForm; asserts the initial hint 'Select a category and enter an amount' (`add_hint_category_amount`) and that submit ('Add') is disabled until a category + positive amount are chosen.
  - ModifyTransaction: preloads via `useFocusLoad(transactionRepository.getById)`; asserts the preloaded form enables Save and that `getTagsByTransactionId(transactionId)` is called.
- Minimal-arg mock declarations (e.g. `vi.fn(async (_filter: unknown) => ...)`) keep typecheck green (a zero-arg fn called with an argument is a TS2554 in the include-set).
- Verification: `test:all` green - typecheck + lint + 71 files / 446 tests (was 63 files / 430 tests). Phase 3 (remaining screens: Categories/AddCategory/CategoryFilter/Data screens etc.) not yet scoped by plan.

[2026-09-04] Refactor | ConfirmationModal dedup (Option C, Feature 1): extract useDeleteConfirmation hook + BulkDeleteConfirmationModal
- New `src/hooks/useDeleteConfirmation.ts`: single-item delete confirmation hook that standardizes the previously-inconsistent modal-closing behavior across the four Modify screens. It returns `{ visible, open, close, confirm }`; `confirm()` closes the modal first, then runs the delete through `runWithErrorAlert` (error prefix + generic alert), then runs `onSuccess` only if the delete succeeded. Modeled exactly on `useBulkDelete` (same error-wrapping + prefix pattern).
- Refactored to the hook (each passes its repo delete as `deleteFn` and the navigate-back/refresh as `onSuccess`):
  - ModifyTagScreen: `deleteFn` = `tagRepository.delete(id)`, `onSuccess` = goBack + deferredRefreshTags. Replaces the inline `onConfirm` wrapper that closed the modal then called `handleDelete`.
  - ModifyCommentScreen: `deleteFn` = `transactionRepository.deleteComment(comment)`, `onSuccess` = goBack. Replaces `handleDelete` (which closed the modal first then ran the delete).
  - ModifyAccountScreen: `deleteFn` = `accountRepository.delete(id)` + `sanitizeDefaultAccounts`/`updateConfig` cleanup, `onSuccess` = goBack + deferredRefreshAccounts. Replaces `handleDeleteConfirm` (which had relied on goBack-unmount to close the modal).
  - TransactionDetailsScreen: `deleteFn` = photo cleanup (`deletePhotoFile` per parsed photo) + `transactionRepository.delete(id)`, `onSuccess` = goBack + deferredRefresh. Replaces the bespoke `handleDelete` with its own try/catch + `deleting` re-entry guard; the modal now closes first so the guard is redundant, and error reporting is unified through `runWithErrorAlert` with `ERROR_PREFIXES.transactionsDelete`. Drops the now-unused `showErrorAlert` import/state.
- New `src/components/BulkDeleteConfirmationModal.tsx`: thin wrapper over `ConfirmationModal` for the identical bulk-delete JSX that four list screens were repeating. Refactored AllTransactionsScreen, TransactionsScreen, CommentsScreen and TagsScreen to use it (same props; no behavior change) with the `useBulkDelete` `{ deleteModalVisible, confirmBulkDelete, closeDeleteModal }` values.
- Deliberately out of scope (documented): the 6 DataScreen usages (step-1/step-2, type-DELETE text confirm, import confirm), the CategoriesScreen/ModifyCategoryScreen `move`/`onMove` + guard-message variants, and AccountsScreen/CategoriesScreen bulk deletes gated behind pre-checks — these stay bespoke.
- Tests: new `tests/hooks/useDeleteConfirmation.test.tsx` (6 tests mirroring `useBulkDelete.test.tsx`: starts hidden, open/close, confirm closes + deletes + onSuccess on success, no onSuccess on reject, routed through runWithErrorAlert, skips onSuccess when absent). Existing ModifyTag/ModifyComment/ModifyAccount/TransactionDetails + list-screen + `useBulkDelete`/`useSelectAndSearch` suites still pass unchanged.
- Verification: `test:all` green - typecheck + lint + 72 files / 452 tests (was 71 files / 446 tests).

[2026-09-04] Refactor | Tier-5 codebase cleanup audit (Option D, Feature 1): dead exports + shared helpers
- Removed 10 dead exports (no public API change); each symbol stays local to its module: `parseRow` (`database/validate.ts`), `BackupSnapshot` (`database/backup.ts`), `ProxyMethod` + `DrizzleRunResult` (`database/drizzle/proxy.ts`), `DrizzleDb` (`database/drizzle/engine.ts`), `TransactionFilters` + `CategoryTagBreakdown` + `CategoryUsageCount` (`database/repositories/transactionRepo.reads.ts`), `TypeTab` (`components/TabBar.tsx`), `UseTransactionListScreenOptions` (`hooks/useTransactionListScreen.tsx`).
- New `src/database/repositories/repoHelpers.ts` with one canonical `existsByName(table, columns, userId, name, excludeId?)`; `accountRepo`/`categoryRepo`/`tagRepo` delegate to it and drop their triplicated LOWER(name) duplicate-check queries (now-unused `ne`/`SQL` imports trimmed). SKIPPED + documented: a shared `patchUpdate` was not worth it - the three `update` scaffolds copy different per-entity fields, so a generic would add indirection without real dedup.
- New `formatAmount(amount, config)` (`utils/formatters.ts`, uses `Pick<Config, 'currency' | 'decimalSeparator'>`); the 9 `formatCurrency(x, config.currency, config.decimalSeparator)` call sites (AccountModal, BarChart, CategoryList, DonutChart, HomeHeader x2, TransactionGroup, AccountsScreen, TransactionDetailsScreen) now call it.
- New private `countByGroup(column, ids)` in `transactionRepo.reads.ts`; `countByCategoryIds` / `countByCategoryIdsMap` / `countByAccountIdsMap` collapsed onto it without changing public names, signatures or results (single COUNT(*) of a filtered set equals the grouped counts summed).
- Hoisted the exactly-matching text style variants into `components/componentStyles.ts`: `LIMIT_TEXT_STYLE` (used by AddCategoryScreen + CategoriesScreen) and `COUNTER_STYLE` (used by TagsScreen + CommentsScreen). Kept local where the style genuinely differs: TagsScreen `limitText` lives inside an absolute `limitWrap` and omits margin/padding; CategoriesScreen `counter` has its own 8/12 padding.
- New `src/hooks/useResetOnOpen.ts` (`useResetOnOpen(visible, reset)` where callers memoize `reset` via `useCallback`, preserving each effect's original dependency semantics exactly). Applied to the 5 modals with the open-time reset pattern: AccountModal, OptionPickerModal, ColorPickerModal, CategoryFilterModal, CalendarModal.
- SKIPPED + documented (would break behavior-preserving rule): routing the three active-flag async effects through `useFocusLoad`. HomeScreen refetches on filter-dep changes (not focus) with a conditional early-return; ModifyCommentScreen runs on mount/prop-change and surfaces errors via `showErrorAlert` (useFocusLoad logs silently); CommentsScreen needs a programmatic reload after bulk delete, which `useFocusLoad` does not expose.
- Cleanup-audit result recorded: nothing to do on i18n (all 409 `en.ts` keys are used, es/ca/fr/de/pt/it parity exact, no missing-key usages). ESLint config intentionally has no import/order rule - import placement stays free.
- Tests: new `tests/hooks/useResetOnOpen.test.tsx` (4 tests: reset on open, no reset while hidden, re-run when the callback changes while visible, no re-run on a no-op rerender) + 3 new `formatAmount` cases in `tests/utils/formatters.test.ts`.
- Verification: `test:all` green - typecheck + lint + 74 files / 464 tests (was 73 files / 457 tests).

[2026-09-06] Upgrade | Expo SDK 54 -> 57 (Expo Go SDK 57 compatibility)
- package.json: expo ^57.0.20 (was ^54), react-native 0.86.3, react 19.2.3, react-dom 19.2.3, react-native-web ~0.21.0. Peer ecosystem aligned to SDK 57 via `npx expo install`: @react-native-community/datetimepicker 9.1.0, react-native-gesture-handler ~2.32.0, reanimated 4.5.1 + react-native-worklets 0.10.1, react-native-screens ~4.26.0, react-native-safe-area-context ~5.7.0, react-native-svg 15.15.4, @expo/vector-icons ^15.0.2, and the expo-* modules at ~57.x. New direct dep expo-splash-screen ~57.0.8 (SDK 57 splash handling). devDeps: eslint-config-expo ~57.0.2, typescript ~6.0.3, @types/react ~19.2.4, @react-native/babel-preset ^0.86.3 (must track react-native, or vitest-native falls back to its mock engine and host nodes change). Removed stale direct deps hermes-compiler ^0.15.1 and @react-native/babel-preset ^0.81.5. `npx expo install --check` reports "Dependencies are up to date"; `expo-doctor@latest` -> 21/21 checks passed.
- FinlyApp/.npmrc (new): `legacy-peer-deps=true`, fixing the npm install ERESOLVE conflicts of the SDK 57 tree on npm 12 / Node 24. package.json gained `allowScripts: { "unrs-resolver": true }` and the user-global `~/.npmrc` reverted to `ignore-scripts=false` only: npm/npx propagate any `allow-scripts` env value into child processes and throw EALLOWSCRIPTS even for allow-listed packages, so the object form (read directly, never env-propagated) is the working fix — both `npm install` and `npx expo install --fix` now succeed.
- app.json: removed the top-level `splash` field (gone from the SDK 57 app schema) and moved splash config into the `expo-splash-screen` plugin: `["expo-splash-screen", { "image": "./assets/splash-icon.png", "resizeMode": "contain", "backgroundColor": "#0F172A" }]`.
- Deleted the stale `android/` directory (SDK 54-era prebuild artifact, gitignored) so the next `expo prebuild` regenerates it from the SDK 57 template.
- src/components/calendars/DayPicker.tsx: replaced `StyleSheet.absoluteFillObject` (removed from RN 0.86 styles types) with an explicit absolute-fill style (typecheck fix).
- eslint.config.js: set to "off" the four React-Compiler-era `react-hooks` rules eslint-config-expo 57 enables (`refs`, `set-state-in-effect`, `immutability`, `preserve-manual-memoization`), since the project does not enable the React Compiler; all other rules stay active (lint green again).
- 11 component test files (EmptyState, ErrorBoundary, Fab, TransactionGroup, IconBadge, AmountInput, SortToggle, PeriodTabs, RadioButton, SearchBar, TabBar): upgraded their local `flattenStyle` to the recursive walk-and-flatten helper that CategoryGrid already had — RN 0.86's real test-renderer returns styles as nested arrays, which the non-recursive flattener missed.
- tests/screens/DataScreen.test.tsx: mocked `expo-sharing` and `expo-document-picker` (native modules) so the suite no longer loads real `expo-modules-core`. That package is a transitive dep of expo@57 whose `main` ships as TypeScript (`src/index.ts`); vitest-native's Node loader cannot strip TS under node_modules, which crashed the DataScreen suite (Cannot find package / named-export interop). A `transform` + alias experiment on `vitest.config.mts` / `tests/mocks/expo-modules-core.ts` was tried and reverted in favor of the module mocks. expo-modules-core must NOT be a direct dependency (expo-doctor flags it).
- Verification: `test:all` green - typecheck + lint + 74 files / 464 tests (DataScreen's 3 tests run again). Web smoke via `npx expo start --web`: app boots; drawer, Home (Total -3,00 euro, Income 0,00 / Expenses 3,00, Transport 100% breakdown) and the AllTransactions list (tabs, sort, period selector, seeded "bus / -3,00 euro" row) all render with 0 console errors — only a benign RN 0.86 `props.pointerEvents` deprecation warning. Target remains Expo Go (SDK 57) on the user's devices.

[2026-09-06] Refactor | silence RN 0.86 InteractionManager deprecation in dev (third-party drawer-layout)
- New `src/utils/devWarningSuppressions.ts`: `LogBox.ignoreLogs(['InteractionManager has been deprecated'])`, imported first in `index.ts` (before `./App`) so it registers before `@react-navigation/drawer` → `react-native-drawer-layout` module evaluation touches the deprecated `InteractionManager` getter. The warning is dev-only (LogBox), runtime unaffected, and upstream's `try/catch` in drawer-layout 4.2.10 only covers future RN removal, not the 0.86 deprecation warn.
- Verification: `test:all` green - typecheck + lint + 74 files / 464 tests.

[2026-09-06] Fix | release-readiness phase A: photo/DB delete ordering, ErrorBoundary + backup i18n, a11y, misc
- Photo-vs-DB delete ordering (data-loss fix): `src/database/photoCleanup.ts` API reshaped into `collectTransactionPhotos(column, ...values)`, `collectAllTransactionPhotos()` and best-effort `deletePhotoUris(uris)` (per-file try/catch — an unlink failure no longer aborts sibling files). All delete flows now capture photo URIs first, delete DB rows inside `withTransaction`, and clean files only after the commit: `transactionRepo.writes` `delete`/`deleteMany`/`deleteAllTransactions`, `accountRepo` `delete`/`deleteMany`, `categoryRepo` `delete`/`deleteMany`/`bulkDeleteWithTargets`. Previously files were removed before the DB delete, losing photos if the row delete failed; critical-path deletes never block on photo bookkeeping (collect is best-effort and returns `[]` on DB read failure).
  - Tests: new `tests/database/photoCleanup.test.ts` (5 tests) and `tests/database/transactionWrites.test.ts` (5 tests, call-order regression incl. "no photo cleanup when the DB transaction fails" and "no bookkeeping for empty deleteMany"). `sqliteContract.test.ts` photoCleanup mock updated to the new API.
- ErrorBoundary i18n: the three hardcoded English strings (title/message/retry) now come from `t()` via new keys `error_boundary_title`/`error_boundary_message`/`error_boundary_retry` in all 7 languages (en/es/ca/fr/de/pt/it). English values unchanged, so the `ErrorBoundary.test.tsx` suite passes as-is.
- Backup error i18n: `BackupValidationError` now carries a typed `code` (`invalid_json` | `invalid_format` | `newer_version`) while keeping its diagnostic English message; `src/screens/settings/DataScreen.tsx` maps the codes to translated alerts and shows a distinct newer-version alert (`settings_import_newer_title`/`settings_import_newer_message`, all 7 languages) instead of the misleading generic invalid-backup message. `tests/database/backup.test.ts` now asserts the three codes.
- a11y: `SelectSearchHeader` search icon is now a real button (`accessibilityRole="button"` + label from new `a11y_search` key, 7 languages); `CommentInput` suggestion rows get `role="button"` + `accessibilityLabel` + `onAccessibilityTap` (responder press handling preserved verbatim so the existing 9-test suite passes unchanged).
...
[2026-09-09] ~ | spec/constitution/3-roadmap.md
- Marked Task 4 done: Finly 2.0.0 released as a GitHub Release (tag v2.0.0) with the EAS-signed preview APK attached — https://github.com/Alfonfdez/Finly/releases/tag/v2.0.0.
...
[2026-09-17] ~ | FinlyApp/scripts/regenerate-icons.mjs, FinlyApp/assets/splash-icon.png, FinlyApp/assets/android-icon-foreground.png, FinlyApp/assets/android-icon-monochrome.png, docs/assets.md
- Regenerated the three branded PNGs with real transparency from the icon.png glyph (previously all PNGs had a baked-in opaque background, and the splash embedded a tiny icon plus the "Finly" text). New script scripts/regenerate-icons.mjs chroma-keys the near-white background out of the glyph and composites it centered on a transparent 1024x1024 canvas: splash-icon is now icon-only (no text) sized ~640 px tall on the #0F172A background; android-icon-foreground sits within the 675 px safe zone on transparency; android-icon-monochrome is a solid white silhouette on transparency (fixes the muddy gray dithered output). Sizes: splash 852->296 KiB, foreground 397->302 KiB, monochrome 53->22 KiB (all < 1 MB NFR).

[2026-09-17] ~ | FinlyApp/scripts/regenerate-icons.mjs, FinlyApp/assets/splash-icon.png, FinlyApp/assets/android-icon-foreground.png, FinlyApp/assets/android-icon-monochrome.png, docs/assets.md
- Resized two of the regenerated PNGs after on-device review: the Android adaptive glyph (foreground + monochrome) was shrinking from the 620 px box to 500 px so it no longer touches the rounded launcher mask (safe zone now 360x496 in a 1024 canvas); the splash glyph grew from 640 px to 760 px tall so it reads larger in the middle of the screen. Transparency, canvas size and SPLASH_GLYPH_WIDTH/FOREGROUND_BOX/MONOCHROME_BOX constants updated in scripts/regenerate-icons.mjs; docs/assets.md values synced. Sizes: splash 296->393 KiB, foreground 302->217 KiB, monochrome 22.5->19 KiB (all < 1 MB).

[2026-09-17] + | FinlyApp/src/i18n/gl.ts, FinlyApp/src/i18n/eu.ts, FinlyApp/src/i18n/index.ts, FinlyApp/src/constants/languages.ts, FinlyApp/src/constants/flagColors.ts, FinlyApp/src/utils/language.ts, FinlyApp/src/database/schemas.ts, FinlyApp/src/screens/settings/RegionalScreen.tsx, FinlyApp/src/utils/formatters.ts
- Added Galego (gl) and Euskara (eu) as full UI languages (9 total). New translation files with full en.ts key parity. Both appear under Catalan in the Regional language dropdown with custom-drawn flags (Galician white + blue diagonal; Basque ikurriña red/green/white), rendered via SVG on every platform like the Catalan senyera. Date handling: Galician uses the default "day de month de year"; Basque got a new year-first branch ("2026ko abuztuak 3"). LANGUAGES, the Zod config enum, and the i18n registry updated.

[2026-09-17] ~ | spec/constitution/1-mission.md, spec/constitution/2-tech-stack.md, spec/features/003-settings-screen/1-spec.md, spec/constitution/3-roadmap.md, AGENTS.md, docs/programming-concepts.md, .gitignore
- Synced the language-count docs from 7 to 9 languages after the Galego/Euskara addition: mission principle, tech-stack file-tree comment, the 003-settings Regional §4.1 table (values + flags) and its acceptance criteria, AGENTS.md (I18N) and programming-concepts.md. Added the roadmap entry "Galego and Euskera languages" (completed). Removed the temporary `.opencode/` Plan-mode folder and added `.opencode/` to `.gitignore` so future plan files never show up as untracked.
- Verification (Playwright MCP, 375px): the Regional language picker lists all 9 languages with Galego and Euskara under Catalan; the custom SVGs render (Galego white rect + blue diagonal, ikurriña red/green/white); selecting Euskara and then Galego relabeled the whole UI immediately (Eskualdea/HIZKUNTZA…, Rexional/IDIOMA…). The new 003 acceptance criterion was flipped to [x]. `npm run test:all` green (92 files / 571 tests).

[2026-09-17] ~ | README.md, README.es.md, README.ca.md, README.fr.md, README.de.md, README.pt.md, README.it.md, README.gl.md, README.eu.md
- Documented Galego/Euskara across the READMEs: added `[Galego](README.gl.md)` and `[Euskara](README.eu.md)` to the language bar and the local names (Gallego/Euskera, Gallec/Basc, Galicien/Basque, Galicisch/Baskisch, Galego/Basco, Galiziano/Basco) to the info table of all 7 existing files, and widened the i18n list from `(en, es, ca, fr, de, pt, it)` to `(en, es, ca, gl, eu, fr, de, pt, it)` in the Tech-Stack and project-layout sections. Created full Galician (`README.gl.md`) and Basque (`README.eu.md`) translations — 166 lines / 14 headings each, same 17 screenshot paths (verified present and identical to the English set).
- Doc sync: `AGENTS.md` project-structure i18n line, `docs/programming-concepts.md` "Centralized language checks" (Language union, LANGUAGES snippet, isCatalan/isGalician/isBasque), `spec/constitution/2-tech-stack.md` utils tree, and `spec/constitution/5-validations.md` i18n-key rule updated from the stale en/es/ca (or 7 languages) to the current 9. Historical per-feature and changelog/roadmap entries left untouched.

[2026-09-17] ~ | FinlyApp/package.json, FinlyApp/package-lock.json, FinlyApp/app.json
- Bumped the Expo SDK 57 patch set to the versions expected by the compat checker via `npx expo install --fix`: expo 57.0.21 -> ~57.0.23, expo-document-picker -> ~57.0.2, expo-image-picker -> ~57.0.18, expo-sharing -> ~57.0.20, expo-splash-screen -> ~57.0.9, expo-sqlite -> ~57.0.3, expo-system-ui -> ~57.0.4 (expo-constants/file-system/font installed to their expected patches without a range change since the old `~` already allowed them). Non-Expo majors/minors deliberately left alone (RN 0.87, react 19.3, babel 8, eslint 10, TypeScript 7, vitest 5, worklets 0.12, gesture-handler 3.x). `expo install` also registered the new `expo-sharing` config plugin (`app.plugin.js` exists in 57.0.20) in `app.json`. Verified: `npm run test:all` green (92 files / 571 tests), `expo-doctor` 21/21, and a 375px web smoke test (Home renders, Settings -> Regional language picker lists all 9 and selecting Euskara relabels to Eskualdea/HIZKUNTZA/DIRU FORMATOA, 0 console errors). `expo install --check` now reports "Dependencies are up to date". Next EAS build should be #3 to include the patches.

[2026-09-18] ~ | FinlyApp/src/components/SelectorInline.tsx, FinlyApp/src/components/componentStyles.ts, FinlyApp/src/screens/settings/AppearanceScreen.tsx, FinlyApp/src/i18n/{en,es,ca,gl,eu,fr,de,pt,it}.ts, FinlyApp/src/screens/settings/RegionalScreen.tsx, tests/screens/RegionalScreen.test.tsx, spec/features/003-settings-screen/1-spec.md, spec/constitution/3-roadmap.md
- Restyled the shared `SelectorInline` option buttons to mirror the Listly app: options are now flexible (fill the row width), centered, bordered (primary border + faint primary tint when selected, neutral border otherwise, `BUTTON_BORDER_RADIUS` 10), equal height via a fixed 20px icon box + `lineHeight: 20` on the option label; the selected checkmark was removed (selection shown by border + tint). Added `PRESSED_OPACITY = 0.7` to `componentStyles.ts` and applied it to the option press feedback. `AppearanceScreen` size-icon "A" glyphs got `lineHeight: 20` so all buttons in the Theme/Text-size rows are the same height.
...
[2026-09-18] + | FinlyApp/modules/finly-share/ (expo-module.config.json, package.json, src/index.ts, android/src/main/AndroidManifest.xml, android/src/main/res/xml/finly_share_paths.xml, android/src/main/java/expo/modules/finlyshare/FinlyShareModule.kt, ios/FinlyShareModule.swift)
- New local Expo module `finly-share`: opens the system share sheet for a local file and resolves `saved`/`dismissed` based on the real outcome (Android `RESULT_CANCELED && data == null`, iOS `completionWithItemsHandler.completed`). FileProvider declared for files in the app documents dir; autolinked from `FinlyApp/modules/`.

[2026-09-18] + | FinlyApp/src/constants/shareResult.ts
- `ShareResult` constants (`SAVED`/`DISMISSED`) and the `ShareResultValue` union type shared by finly-share, backupIO and DataScreen (no magic strings in JS).

[2026-09-18] ~ | FinlyApp/src/utils/backupIO.ts, FinlyApp/src/utils/backupIO.web.ts
- `saveBackupFile` now returns `ShareResultValue` instead of `void`: native path shares via `finly-share` after writing the file to documents (expo-sharing keeps its `isAvailableAsync` availability gate), web path always reports `saved` (download already happened).

[2026-09-18] ~ | FinlyApp/src/screens/settings/DataScreen.tsx
- Export feedback fixed: the success alert fires only when the share really completed (`ShareResult.SAVED`); dismissing the native share sheet shows no alert; failures still show the error alert. Web unchanged (Alert is a no-op there).

[2026-09-18] ~ | FinlyApp/tests/screens/DataScreen.test.tsx
- Added export coverage (completed share -> success alert, dismissed share -> no alert, failure -> error alert) and import coverage (picker cancel -> no confirm modal/alert, confirmed import -> success alert). Fixed the pre-existing broken `backupIO` mock path (`../../utils/backupIO` -> `../../src/utils/backupIO`) that had never been exercised.

[2026-09-18] ~ | spec/features/025-data-backup/1-spec.md, spec/constitution/3-roadmap.md
- 025 spec: native export now reports the share-sheet outcome (success alert only when completed; dismissed shows nothing; errors show the error alert) and the Export row icon matches the code (share-outline). Roadmap 025 entry notes the native share-result fix + tests.

[2026-09-18] ~ | FinlyApp/modules/finly-share/android/build.gradle, FinlyApp/modules/finly-share/src/index.ts
- Fixed EAS build #5 startup crash: the local module was missing its Gradle entry, so expo-modules-autolinking's Android resolver dropped it (isAndroidProject filter) - the JS wrapper was bundled but the native module/FileProvider were not compiled/merged, and the eager requireNativeModule threw at launch. Added the canonical android/build.gradle (com.android.library + expo-module-gradle-plugin, namespace expo.modules.finlyshare) so the module is linked, compiled, and its manifest merged. Hardened src/index.ts: requireNativeModule is now lazy + try/catch (app can never crash at startup if the native side is missing; errors surface only when exporting). Autolinking verified: `resolve --platform android` now lists finly-share (sourceDir modules/finly-share/android, classifier expo.modules.finlyshare.FinlyShareModule). npm run test:all green (92 files / 576 tests). Rebuild submitted as EAS #6 (b634e353-7316-475d-b56d-01fb6ba8e8c2).

[2026-09-18] ~ | FinlyApp/modules/finly-share/android/src/main/java/expo/modules/finlyshare/FinlyShareModule.kt, docs/../spec/features/025-data-backup/1-spec.md
- Android share-result reality: device test showed a successfully completed Gmail share returned RESULT_CANCELED, so the strict "saved only on RESULT_OK" mapping hid the success alert. Android share targets do not return a reliable result (most finish without setResult), so the module now reports `saved` whenever the share sheet ran and returns (activity return for the request code), dropping the RESULT_CANCELED -> `dismissed` mapping and the now-unused SHARE_RESULT_DISMISSED constant + Activity import. iOS keeps truthful detection via completionWithItemsHandler.completed (real dismiss still shows no alert); web unchanged. Tradeoff: backing out of the Android share sheet also shows the success alert, but the backup file was exported regardless. FinlyApp/.gitignore now ignores modules/**/android/build/ (module Gradle outputs, ~168 files). Local release APK rebuilt (incremental, 18s; debug keystore unchanged so it installs over the previous test APK) and re-verified (FinlyShareModule in classes2.dex, finlyshare.fileprovider in manifest). npm run test:all green (92 files / 576 tests). 025 spec updated to document the per-platform feedback behavior. Roadmap 025 entry notes the Android limitation.

[2026-09-18] + | FinlyApp/modules/finly-share/android/src/main/java/expo/modules/finlyshare/FinlyShareModule.kt, FinlyApp/modules/finly-share/src/index.ts, FinlyApp/src/utils/platform.ts, FinlyApp/src/utils/backupIO.ts, FinlyApp/src/utils/backupIO.web.ts, FinlyApp/src/screens/settings/DataScreen.tsx, FinlyApp/src/i18n/*.ts
- Hybrid export on Android: pressing Export now writes `finly-backup-YYYY-MM-DD.json` to the **public Downloads folder** via the new `finly-share.saveToDownloadsAsync` (MediaStore.Downloads, IS_PENDING write, Android 10+; returns false on older Android or if the native module is missing). The alert is shown only after the write succeeds: "Your backup has been saved to the Downloads folder" with **[Share]** and **[Done]** actions. Share is optional and fire-and-forget (`saveBackupFile`, existing documents copy + share sheet) and its outcome is never reported — Android share targets return no reliable result, so no modal can ever be false. If the Downloads write is unsupported, the export falls back to the previous share flow so it always works. iOS keeps the truthful share-sheet outcome; web unchanged (browser download).
- Added `isAndroid` and `isAndroidPlatform()` to src/utils/platform.ts (runtime helper so tests can switch Platform.OS); `saveBackupToDownloads` to backupIO.ts (Android-only) and backupIO.web.ts (downloads and returns true); new i18n keys settings_export_downloaded_title/_message/_share_action/_done_action in all 9 languages. DataScreen tests cover the Android flow (Downloads alert with Share/Done, Share opens the share sheet, fallback when unsupported) plus platform.test additions. npm run test:all green (92 files / 579 tests). 025 spec + roadmap updated for the hybrid Android Downloads flow.

[2026-09-18] Release | 2.1.0
- Version bump 2.0.0 -> 2.1.0: app.json `version` 2.1.0, new `android.versionCode` 2 (was unset, i.e. 1) and `ios.buildNumber` 2.1.0; `package.json`/`package-lock.json` root versions to 2.1.0. The Drawer footer version (`src/navigation/AppNavigator.tsx`, `Constants.expoConfig?.version`) picks up 2.1.0 automatically with no code change.

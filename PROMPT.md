...
4. Read `docs/` files for changelog, git workflows, programming concepts, and app assets reference.
...
8. Any code update must be documented in `docs/changelog.md`.
9. When asked for a commit message, follow the workflow in `docs/git-commands.md` (run `git status` + `git diff --stat`, then generate conventional commit + branch name).
10. Always consult the developer before adding, modifying, or deleting code. Wait for verification.
